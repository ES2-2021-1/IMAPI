...
export default {
    ...
    methods: {
        async fetchModules() {
            let { data } = await window.axios.get(`/api/module/`, {headers: { Authorization: `Bearer ${this.$session.get("jwt")}`}}).catch(() => this.$router.push({ name: 'error' }));
            this.modules = data;
        },
        async fetchStartup() {
            let { data } = await window.axios.get(`/api/startup/`+this.id, {headers: { Authorization: `Bearer ${this.$session.get("jwt")}`}}).catch(() => this.$router.push({ name: 'error' }));
            this.startup = data[0];
            if(!this.startup || this.startup.ownerId != this.$session.get('userId')){
                this.$router.push({ name: 'error' });
            }
        },
        async fetchSteps() {
            let { data } = await window.axios.get(`/api/step/`, {headers: { Authorization: `Bearer ${this.$session.get("jwt")}`}}).catch(() => this.$router.push({ name: 'error' }));
